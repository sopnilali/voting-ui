...
import { useCreateElectionMutation } from '../../../../../Redux/features/election/electionApi';
import { toast } from 'sonner';

export const CreateElectionModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createElection(formData).unwrap();
      if (result) {
        toast.success('Election created successfully');
        onClose();
      }
    } catch (error) {
      toast.error(error?.data?.message || 'Failed to create election');
    }
  };
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <h3 className="text-xl font-bold text-gray-200 mb-4">Create New Election</h3>
